...
def prepare_data_for_ner(filename, columns, sentence_column="Sentence", ner_label="OUTCOME", sent_split=True):
    all_data = []
    y = []
...
def train_NER(train_data, eval_data, args):
    ner_model = None
    if args.global_model_to_train.strip():
        ner_model = spacy.load(args.global_model_to_train)  # load existing spaCy model
        print("Loaded model '%s'" % args.global_model_to_train)
    else:
    ...
    if not args.global_model_to_train.strip():
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sent_split', default="True")
    parser.add_argument('--global_model_to_train', default="")
    ...
    print("Global model to train: %s"%args.global_model_to_train)
    args.n_iter = int(args.n_iter)
    args.test_num = int(args.test_num)
    args.sent_split = True if args.sent_split.lower() == "true" else False
    ...
    if os.path.exists(train_data_file):
        ...
        for file in os.listdir(args.folder_with_dataset):
            _all_data, _y = prepare_data_for_ner(
                os.path.join(args.folder_with_dataset, file), classes_to_take, sentence_column=args.sentence_column,
                ner_label=args.ner_label, sent_split=args.sent_split)
