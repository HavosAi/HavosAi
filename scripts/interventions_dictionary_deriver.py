...
if __name__ == '__main__':
    ...
    parser.add_argument('--label_interventions', default='True')
    ...
    print("Label interventions: %s"%args.label_interventions)
    print("Derive taxonomy: %s"%args.derive_taxonomy)

    if args.output_predicted_interv_folder:
        os.makedirs(args.output_predicted_interv_folder, exist_ok=True)

    if args.label_interventions.lower() == "true":
        ...
        interventions_df_full = pd.DataFrame()
        for filename in os.listdir(args.folder_dataset):
            if os.path.isdir(os.path.join(args.folder_dataset, filename)):
                continue
            interventions_df = pd.read_excel(os.path.join(args.folder_dataset, filename)).fillna("")
            if "Predicted Label" not in interventions_df.columns:
                interventions_df["Predicted Label"] = interventions_df["Label"]
            interventions_df_full = pd.concat([interventions_df_full, interventions_df], axis=0)
    ...
    if args.derive_taxonomy.lower() == "true":
        ...
        _dict_deriver = interventions_dictionary_deriver.InterventionsDictionaryDeriver(synonyms_model_dir=args.word2vec_model_folder)
