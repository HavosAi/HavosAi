...
import os
from text_processing import text_normalizer, column_filler
...
class FullLogicStudytypeLabeler:
    ...
    def __init__(self):
        self.exact_keywords = {"systematic review":"Systematic review", "meta analysis":"Meta analysis", "chapter":"Book chapter",\
                  ...
                  "laboratory study":"Laboratory study","laboratory experiment":"Laboratory study","lab study":"Laboratory study","lab experiment":"Laboratory study",\
                  "lab condition":"Laboratory study","laboratory condition":"Laboratory study","laboratory scale":"Laboratory study","lab scale":"Laboratory study",\
                  "modeling study":"Modeling study", "simulation study":"Modeling study", "analysis study":"Modeling study",\
                  "participant study":"Observational study", "respondent study":"Observational study", "population study":"Observational study","farmer study":"Observational study",
                  "smallholder study":"Observational study",\
                  "participant survey":"Observational study", "respondent survey":"Observational study", "population survey":"Observational study","farmer survey":"Observational study",\
                  "smallholder survey":"Observational study",\
                  "randomized controlled trial": "Randomized controlled trials", "RCT": "Randomized controlled trials", "quasi experimental": "Randomized controlled trials",\
                  "propensity score":"Observational study", "endogenous switching":"Observational study", "comparison study":"Observational study", "case study": "Observational study",\
                  "impact evaluation":"Impact evaluation", "distributional impact":"Observational study",\
                  "randomized controlled":"Observational study","ex post facto": "Observational study", "ESRMs": "Observational study",\
        ...
        self.key_words = {"systematic review":"Systematic review", "meta analysis":"Meta analysis", "chapter":"Book chapter",\
              ...
              "randomized controlled trial": "Observational study", "RCT": "Observational study", "quasi experimental design": "Observational study","propensity score":"Observational study",\
              "endogenous switching":"Observational study", "impact evaluation":"Observational study", "comparison study":"Observational study", "distributional impact":"Observational study",\
    ...
    def label_df_with_study_type(self, articles_df, search_engine_inverted_index, use_prediction = False, gpu_device_num = 0,
            ...
            file_with_keywords="../data/study_design_types.xlsx",
            model_with_agg=False):
        _colum_filler = column_filler.ColumnFiller(file_with_keywords, keyword_column="Keyword", high_level_label_column="High level label")
        articles_df = _colum_filler.label_articles_with_outcomes("study_type", articles_df, search_engine_inverted_index, None,
            label_details_column="keywords_study_type", keep_hierarchy=False, resolve_abbreviations=False)
        for i in range(len(articles_df)):
            articles_df["keywords_study_type"].values[i] = list(articles_df["keywords_study_type"].values[i])
            articles_df["study_type"].values[i] = list(articles_df["study_type"].values[i])
            if len(articles_df["study_type"].values[i]) == 0:
                articles_df["study_type"].values[i] = ['No category']
        study_type_labels = ["Observational study", "Modelling study", "Laboratory study", "Review paper", "Field study",
                "Economic simulation", "Landscape/environmental simulation"]
        if use_prediction:
            model_type = "with_agg" if model_with_agg else "without_agg"
            model_classes = {
                "without_agg": study_type_labeler.StudyTypeLabeler,
                "with_agg": study_type_labeler_window_agg.StudyTypeLabeler
            }
            _study_type_labeler = model_classes[model_type](
                  folder,
