...
class AuthorAffiliationExtractor:
    ...
    def are_affiliations_similar(self, first_affil, second_affil, without_country = False, threshold = 0.88):
        ...
        return utils.normalized_levenshtein_score(first_affil, second_affil) >= threshold or\
                utils.normalized_levenshtein_score(old_first_affil, old_second_affil) >= threshold or\
                utils.normalized_levenshtein_score(self.sorted_string(first_affil), self.sorted_string(second_affil)) >= threshold\
                or utils.normalized_levenshtein_score(
