...
class DuplicateFinder:
    ...
    def __init__(self, duplicate_info_folder, title_column = "title", abstract_column = "abstract", year_column = "year", dataset_column = "dataset", additional_columns=[]):
    ...
    def are_articles_similar(self, first_doc, second_doc, second_doc_id, tfidf_matrix, tfidf_matrix_abstract, df1, df2):
        ...
        if utils.normalized_levenshtein_score(first_title, second_title) >= 0.9 or cosine_similarity(tfidf_matrix[first_doc], tfidf_matrix[second_doc]) >= 0.95:
            if cosine_similarity(tfidf_matrix_abstract[first_doc], tfidf_matrix_abstract[second_doc]) >= 0.85:
                return True
            elif df1[self.abstract_column].values[first_doc].strip() == "" or df2[self.abstract_column].values[second_doc_id].strip() == "":
                all_match = True
                for column in self.additional_columns:
                    if column in df1.columns and column in df2.columns:
                        if utils.normalized_levenshtein_score(df1[column].values[first_doc], df2[column].values[second_doc_id]) < 0.85:
                            all_match = False
                if all_match:
                    return True
        if cosine_similarity(tfidf_matrix[first_doc], tfidf_matrix[second_doc]) > 0.8 and \
    ...
    def get_duplicate_articles_by_title(self, first_set, second_set, tfidf_matrix, tfidf_matrix_abstract, df1, df2):
        ...
        for first_doc in first_set:
            for second_doc in second_set:
                ...
                if self.are_articles_similar(first_doc, second_doc, second_doc_id, tfidf_matrix, tfidf_matrix_abstract, df1, df2):
    ...
    def get_pair_if_duplicate(self, i,j, tfidf_matrix, tfidf_matrix_abstract, df):
        if self.are_articles_similar(i, j, j, tfidf_matrix, tfidf_matrix_abstract, df, df):
            if not df[self.abstract_column].values[j].strip() and df[self.abstract_column].values[i].strip():
                return (j, i)
            if not df[self.abstract_column].values[i].strip() and df[self.abstract_column].values[j].strip():
                return (i, j)
