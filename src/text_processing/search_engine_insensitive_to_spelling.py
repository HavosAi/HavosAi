...
class SearchEngineInsensitiveToSpelling:
    ...
    def calculate_abbreviations_count_docs(self, articles_df):
        ...
        for idx, abbr in enumerate(self._abbreviations_resolver.resolved_abbreviations):
            ...
            for abbr_meaning in self._abbreviations_resolver.resolved_abbreviations[abbr]:
                docs_for_abbr_meaning = self.find_articles_with_keywords([abbr_meaning], 0.92, extend_with_abbreviations=False, check_all_words=False)
    ...
    def find_similar_words_by_spelling(self, word, threshold = 0.85, all_similar_words = False, check_all_words=True):
        time_total = time()
        ...
        words.add(stemmed_word.replace(" ", ""))
        ...
        if check_all_words:
            ...
            try:
                for w in words:
                    word_parts = w.split(" ")
                    word_parts_extended = [set([word_parts[i]]) for i in range(len(word_parts))]
                    for idx, word_part in enumerate(word_parts):
                        articles_count = len(self.get_articles_by_word(word_part))
                        for dict_word in self.dictionary_by_first_letters[word_part[:self.symbols_count]]:
                            if " " in dict_word:
                                continue
                            if all_similar_words or (articles_count == 0 or len(self.get_articles_by_word(dict_word)) < 4*articles_count):
                                if utils.normalized_levenshtein_score(dict_word, word_part) >= (threshold - 0.2):
                                    word_parts_extended[idx].add(dict_word)
                    queries_to_check = word_parts_extended[0]
                    new_queries = []
                    for i in range(1, len(word_parts_extended)):
                        for query_1 in word_parts_extended[i]:
                            for query_old in queries_to_check:
                                new_queries.append(query_old + " " + query_1)
                        queries_to_check = new_queries
                        new_queries = []
                    for query in queries_to_check:
                        if utils.normalized_levenshtein_score(query, w) >= threshold and len(self.get_articles_by_word(query)) > 0:
                            final_words.add(query)
                    final_words.add(w)
            except:
                pass
        return final_words.union(z_s_replaced_words)
    ...
    def find_articles_with_keywords(self, key_words, threshold = 0.85, extend_query = False, extend_with_abbreviations = True, extend_abbr_meanings = "",check_all_words=True):
        total_articles = set()
        time_start = time()
        time_total = time()
        for key in key_words:
            normalized_key = text_normalizer.normalize_text(key)
            normalized_key = text_normalizer.remove_accented_chars(normalized_key)
            ...
            for query in extended_queries:
                ...
                for key_word in self.find_keywords(text_normalizer.get_stemmed_words_inverted_index(query)):
                    ...
                    for sim_word in self.find_similar_words_by_spelling(key_word, threshold,check_all_words=check_all_words):
