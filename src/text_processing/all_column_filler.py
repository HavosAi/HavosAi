...
class AllColumnFiller():
    ...
    def __init__(self, log_status_filename = ""):
        self.column_classes = {
            ...
            "NERExtractor": self.extract_ner_entities
        }
    ...
    def fill_geo_names(self, articles_df, search_engine_inverted_index, _abbreviations_resolver, column_info, status_logger = None):
        countries_finder = geo_names_finder.GeoNameFinder(
            geo_regions_file=("../data/GeoRegions.xlsx" if "geo_regions_file" not in column_info else column_info["geo_regions_file"]),
            world_bank_regions_file=("../data/WorldBankCountriesDivision.xlsx" if "world_bank_regions_file" not in column_info else column_info["world_bank_regions_file"]),
            provinces_file=("../data/provinces.xlsx" if "provinces_file" not in column_info else column_info["provinces_file"]),
            districts_file=("../data/districts.xlsx" if "districts_file" not in column_info else column_info["districts_file"]),
            additional_countries_file=("../data/map_additional_country_names.xlsx" if "additional_countries_file" not in column_info else column_info["additional_countries_file"]),
            additional_geo_regions_file=("../data/map_additional_regions.xlsx" if "additional_geo_regions_file" not in column_info else column_info["additional_geo_regions_file"]),
            country_adjectives_file=("../data/map_country_adjectives.xlsx" if "country_adjectives_file" not in column_info else column_info["country_adjectives_file"]),
            currencies_file=("../data/map_currencies.xlsx" if "currencies_file" not in column_info else column_info["currencies_file"]),
            country_groups_file=("../data/map_country_groups.xlsx" if "country_groups_file" not in column_info else column_info["country_groups_file"]),
            country_groups_income_level_file=("../data/map_country_groups_income_level.xlsx" if "country_groups_income_level_file" not in column_info else column_info["country_groups_income_level_file"]))
        articles_df = countries_finder.label_articles_with_geo_names(articles_df, search_engine_inverted_index,\
         ...
         check_only_country_names_and_divisions=False if "check_only_country_names_and_divisions" not in column_info else column_info["check_only_country_names_and_divisions"])
        return articles_df
    ...
    def extract_ner_entities(self, articles_df, search_engine_inverted_index, _abbreviations_resolver, column_info, status_logger = None):
        _ner_entity_extractor = ner_entity_extractor.NEREntityExtractor(model_folder = column_info["model_folder"])
        articles_df = _ner_entity_extractor.find_ner_entities(
            articles_df, columns_to_process=column_info["columns_to_process"],
            column_name=column_info["column_name"], entity_labels=[] if "entity_labels" not in column_info else column_info["entity_labels"])
        return articles_df
    ...
    def fill_population_tags(self, articles_df, search_engine_inverted_index, _abbreviations_resolver, column_info, status_logger = None):
        ...
        articles_df = _population_tags_finder.label_with_population_tags(articles_df, search_engine_inverted_index,
            column_name=("population tags" if "column_name" not in column_info else column_info["column_name"]))
        return articles_df
    ...
    def fill_study_type(self, articles_df, search_engine_inverted_index, _abbreviations_resolver, column_info, status_logger = None):
        ...
        articles_df = _study_type_labeller.label_df_with_study_type(articles_df, search_engine_inverted_index, use_prediction = column_info["use_prediction"] if "use_prediction" in column_info else True,\
         gpu_device_num = column_info["gpu_device_num"] if "gpu_device_num" in column_info else 0,
         ...
         model_with_agg=True if "model_with_agg" not in column_info else column_info["model_with_agg"])
        return articles_df
