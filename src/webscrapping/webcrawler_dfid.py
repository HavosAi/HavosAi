...
class WebCrawlerDFID(WebCrawlerBase):
	...
	def extract_links(self, doc):
		return [ item['href'] for item in doc.select('li.document a')]    
	...
	def fill_df_fields(self, meta_doc, df, i):
	    ...
	    if len(meta_doc.select("#abstract")) > 0:
	        df["abstract"].values[i] = meta_doc.select("#abstract")[0].find_next_sibling("p").text
