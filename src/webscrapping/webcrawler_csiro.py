...
class WebCrawlerCSIRO(WebCrawlerBase):
    ...
    def extract_links(self, doc):
        return [item['href'] for item in doc.select('div.results-excerpts a')]

    def process_article(self, article_url, folder_to_save):
        print(article_url)
        if re.search("PIcsiro:[^\/]*",article_url) != None:
            article_id = re.search("(PIcsiro:)([^\/]*)", article_url).group(1)
            self.fetch(
                self.domain_name + "/" + article_url[1:],
