...
class InterventionsDictionaryDeriver:
    ...
    def __init__(self, synonyms_model_dir, middle_layer_keywords = None, intervention_words=None):
        ...
        if middle_layer_keywords is None:
            ...
            "Technology intervention": ["soil", "fertilizer", "water", "irrigation", "tillage", "genetic", "seed", "crop", "plant breeding", "milking","livestock breeding", "livestock", "energy", "health","nutrition","weather", "mobile"],
        ...
        self.middle_layers_to_rename = {
            "milking": "livestock",
            "livestock breeding": "livestock"
        }
        
        self.synonyms_model = synonyms_processor.SynonymsProcessor(synonyms_model_dir)

    def get_embed_for_phrase(self, text):
        for w in ['programme',
                 'policy',
                 'action',
                 'program',
                 ...
                 'reform']:
            text = text.replace(w, " ")
        return self.synonyms_model.get_average_embedding(text)
    ...
    def remove_intervention_synonyms_from_concepts(self, _abbreviations_resolver):
        ...
        for w in new_dict:
            ...
            if firstpart in new_dict and last_part in self.synonyms_model.google_model:
                if self.synonyms_model.google_model.similarity(last_part, "approach") > 0.3 or self.synonyms_model.google_model.similarity(last_part, "intervention") > 0.3:
        ...
        for narrow_concept in self.interventions_set:
            for abbr_meaning in self.interventions_set[narrow_concept]:
                if self.interventions_set[narrow_concept][abbr_meaning] == interv_class:
                    ...
                    for broad_con in text_normalizer.get_bigrams(narrow_concept_to_check, self.synonyms_model.phrases):
    ...
    def find_most_similar_to_concepts(self, word, words_to_check):
        max_similarity = 0.0
        word_embed = self.get_embed_for_phrase(word)
        most_similar_word = ""
        for word_compare in words_to_check:
            sim = round(1 - spatial.distance.cosine(word_embed, self.get_embed_for_phrase(word_compare)), 2)
            if sim > max_similarity:
                max_similarity = sim
                most_similar_word = word_compare
        ...
        for w in dict_w:
            if len(dict_w[w]) > 1:
                ...
                for middle_w in dict_w[w]:
                    words_in_concept = [
                        wf for wf in text_normalizer.get_bigrams(w[2], self.synonyms_model.phrases) if wf in self.synonyms_model.google_model]
                    if len(words_in_concept) > 0:
                        most_sim_word, similarity = self.find_most_similar_to_concepts(middle_w, words_in_concept)
                        full_res.append((middle_w, similarity))
                    else:
    ...
    def find_middle_layer_names(self, _abbreviations_resolver):
        ...
        for interv_class in self.middle_layer_keywords:
            ...
            for w in technology_dict_nar:
                chosen_word, similarity = self.find_most_similar_to_concepts(w, self.middle_layer_keywords[interv_class])
                if similarity < 0.3:
            ...
            for narrow_concept, abbr_meaning, narrow_concept_to_check in dict_w:
                ...
                middle_layer_names = set()
                for middle_layer_name in dict_w[(narrow_concept, abbr_meaning, narrow_concept_to_check)]:
                    if middle_layer_name in self.middle_layers_to_rename:
                        middle_layer_name = self.middle_layers_to_rename[middle_layer_name]
                    middle_layer_names.add(middle_layer_name)
                self.new_interventions_set[narrow_concept][abbr_meaning] = (self.interventions_set[narrow_concept][abbr_meaning], middle_layer_names)