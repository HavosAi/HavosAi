...
def add_responsible_persons(full_df, persons=["Sarah", "Sammi"]):
    full_df["labelled"] = 0
    full_df["id"] = 0
    ...
    full_df["responsible_person"] = ""
    proportion_num = int(len(all_ids) / len(persons))
    for i, person in enumerate(persons):
        end = (i+1) * proportion_num
        if i == (len(persons) - 1):
            end = len(all_ids)
        print(i*proportion_num, end)
        for idx in all_ids[i*proportion_num: end]:
            full_df["responsible_person"].values[idx] = person
    return full_df
